	...
	function index($req) {
		...
		while($row = mysql_fetch_array($result,MYSQL_ASSOC)) {
			...
			if($row['Key'] != "PRI") {
				...
				// set independent id filed name for s.model
				$id_field="'{\$req['".$row['Field']."']}'";
			}
		...
		// set independent id field name for s.model, s.edit
		$nfields = $fields;
		$id = array_shift($nfields);
		$id = trim($id, '{}')
